...
// },[])
    ...
    return (
        ...
        <div style={{backgroundImage: `url(${banner})`,color: 'white', height: 'auto', padding: '1rem', }}><div style={{textAlign: 'left', padding: '2rem'}}><div ></div><div style={{fontSize: '2rem'}}>DevOps</div></div>
    ...
    {CardsJson?.schema[0]?.DevOps[0].ImportantFeatures.map((c) => {
        return (
...
{c.Name}
           ...
           Getting Started as a <span style={{color: '#ee0000'}}>DevOps . . .</span></div> 
           
            <div>
            <Box>
    <Grid container spacing={2}>

    {CardsJson?.schema[0]?.DevOps.map((c,i) => { 
